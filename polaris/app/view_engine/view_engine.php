...
class pl_template_engine
{
	...
	public function render_template( $template )
	{
		...
		{
			// Capturamos el contenido del script sin mandarlo al navegador
			ob_start();
			...
			if( $this->cache_enabled )
			{
				...
				// Enviamos el contenido del buffer al navegador ( ob_get_flush )
			...
			{
				// En el caso de que la caché no esté habilitada, limpia el buffer de salida ( el del navegador), y no envía nada al cliente.
				ob_end_clean();
			}
